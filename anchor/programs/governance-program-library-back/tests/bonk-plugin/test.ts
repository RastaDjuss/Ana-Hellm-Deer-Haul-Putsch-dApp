...
describe('bonk-plugin', () => {
  const governanceProgramId = new anchor.web3.PublicKey(
    'GovER5Lthms3bLBqWub97yVrMmEogzX7xNjdXpPPCVZw',
  );
  const realm = new anchor.web3.PublicKey(
    'A2MqSD5iEqTjBkTg9naHoS5x1YLhQemrJTcBEFXoasph',
  );
  const stakePool = new anchor.web3.PublicKey(
    '9YbGRA8qfDeJuonKJuZYzDXcRVyB2B6cMPCWpTeC1eRL',
  );
  //8TPev5NsgJoViPrRUXp7F1HjUhmHDjzknG4i82FPVWkT
  const governingTokenMint = new anchor.web3.PublicKey(
    '6WddAU5ryy4BSNWqNPM3LsPYcirKY2ax7U2yfkzwe1kq',
  );
  const governingTokenOwner = wallet.publicKey;
  const previousVoterWeightPluginProgramId = new anchor.web3.PublicKey(
    'BUsq2cFH6cmfYcoveaf52BkPaXUW3ZhTUnFybyaJrtkN',
  );

  const governanceKey = new anchor.web3.PublicKey(
    'GyzFPcYaG6wfoi7Y3jLxAvXZWDR3nNqni1cJxkTSQHuq',
  );
  const proposalKey = new anchor.web3.PublicKey(
    'AmGnTWwrJSszBgtRTA3j4pRL6o9GNsBuEQj3UY85AQwB',
  );
  const ownerAta = token.associatedAddress({
    mint: governingTokenMint,
    owner: keypair.publicKey,
  });
  const stakeMintToken = new anchor.web3.PublicKey(
    'J5d7DVTTdGj7KcDtuFWL3322rasxHjpHhUmEzqFLHjuB',
  );
  const vaultAta = '5DpomEty6Rgpe46wu8dyCs8bQviFJdo2yRbxBBxH8NbM';
  const ownerStakeMintAta = token.associatedAddress({
    mint: stakeMintToken,
    owner: keypair.publicKey,
  });
  ...
  const [registrar] = anchor.web3.PublicKey.findProgramAddressSync(
    [Buffer.from('registrar'), realm.toBytes(), governingTokenMint.toBytes()],
    program.programId,
  );
  ...
  const [inputVoterWeight] = anchor.web3.PublicKey.findProgramAddressSync(
    [
      new anchor.web3.PublicKey(
        'D5UGhWTuEoDzgk5NFJL1zdeogV5HP3Fa9VT5Ep12yzXG',
      ).toBytes(),
      Buffer.from('voter-weight-record'),
      governingTokenOwner.toBytes(),
    ],
  ...
  const [stakeDepositRecord] = anchor.web3.PublicKey.findProgramAddressSync(
    [Buffer.from('stake-deposit-record'), voterWeightRecord.toBytes()],
    program.programId,
  );
  ...
  it('Is initializes registrar!', async () => {
    try {
      ...
      const sig = await sendAndConfirmTransaction(web3Connection, txx, [
        keypair,
      ]);
      console.log(sig);
  ...
  xit('creates voter weight record', async () => {
    const tx = await program.methods
      .createVoterWeightRecord(governingTokenOwner)
      .accountsPartial({
        voterWeightRecord,
        registrar,
        stakeDepositRecord,
  ...
  xit('updates voter weight record', async () => {
    const updateVwrIx = await program.methods
      .updateVoterWeightRecord(stakeDepositReceipts.length, proposalKey, {
        createProposal: {},
      })
      .accounts({
        registrar,
  ...
  xit('fetches Voter Weight Record', async () => {
    const vwrInfo = await program.account.voterWeightRecord.fetch(
      voterWeightRecord,
    );
    console.log(vwrInfo);
    console.log('THe current power is: ', vwrInfo.voterWeight.toNumber());

    const sdrInfo = await program.account.stakeDepositRecord.fetch(
      stakeDepositRecord,
    );
    console.log(sdrInfo);
  });
  ...
  xit('creates stake deposit receipt', async () => {
    ...
    try {
      const ixs = await Promise.all(
        stakeDepositReceipts.map(async (nn, i) => {
          return await stakeProgram.methods
            .deposit(i + 15, new anchor.BN(1000), new anchor.BN(364000))
            .accounts({
              owner: keypair.publicKey,
              from: ownerAta,
              vault: vaultAta,
      ...
      const sig = await sendAndConfirmTransaction(web3Connection, tx, [
        keypair,
      ]);
      console.log(sig);
    } catch (e) {
