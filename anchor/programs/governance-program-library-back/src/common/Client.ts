...
export const DEFAULT_GOVERNANCE_PROGRAM_ID = new PublicKey(
...
export abstract class Client<T extends Idl> {
  ...
  abstract createVoterWeightRecord(
    voter: PublicKey,
    realm: PublicKey,
    mint: PublicKey,
  ): Promise<TransactionInstruction | null>;
  abstract createMaxVoterWeightRecord(
    realm: PublicKey,
    mint: PublicKey,
  ): Promise<TransactionInstruction | null>;
  abstract updateVoterWeightRecord(
    voter: PublicKey,
    realm: PublicKey,
    mint: PublicKey,
  ...
  abstract updateMaxVoterWeightRecord(
    realm: PublicKey,
    mint: PublicKey,
    action?: VoterWeightAction,
    inputRecordCallback?: () => Promise<PublicKey>,
  ...
  async getRegistrarAccount(realm: PublicKey, mint: PublicKey) {
    ...
    const registrarObject = await (
      this.program.account as PluginProgramAccounts<T>
    ).registrar.fetchNullable(registrar);
    ...
    return registrarObject as
      | (IdlAccounts<T>['registrar'] & {
  ...
  async getVoterWeightRecord(
    ...
    mint: PublicKey,
    walletPk: PublicKey,
  ): Promise<IdlAccounts<T>['voterWeightRecord'] | null> {
    ...
    const voterWeightRecord = await voterWeightRecordAccount.fetchNullable(
      voterWeightPk,
    );
  ...
  async getMaxVoterWeightRecord(realm: PublicKey, mint: PublicKey) {
    const { maxVoterWeightPk } =
      (await this.getMaxVoterWeightRecordPDA(realm, mint)) ?? {};
    ...
    const maxVoterWeightRecord =
      await maxVoterWeightRecordAccount.fetchNullable(maxVoterWeightPk);
  ...
  protected async getPredecessorVoterWeightRecordPDA(
    realm: PublicKey,
    mint: PublicKey,
    voter: PublicKey,
    inputRecordCallback?: () => Promise<PublicKey>,
    ...
    const inputVoterWeight = await this.derivePredecessorVoterWeightRecordPDA(
      realm,
      mint,
      voter,
    );
  ...
  protected static deriveVoterWeightRecordPDAForProgram(
    realm: PublicKey,
    mint: PublicKey,
    walletPk: PublicKey,
    programId: PublicKey,
  ...
  protected static getMaxVoterWeightRecordPDAForProgram(
    realm: PublicKey,
    mint: PublicKey,
    programId: PublicKey,
  ): {
    ...
    const [maxVoterWeightPk, maxVoterWeightRecordBump] =
      PublicKey.findProgramAddressSync(
        [
  ...
  async getVoterWeightRecordPDA(
    ...
    return Client.deriveVoterWeightRecordPDAForProgram(
      realm,
      mint,
      walletPk,
      this.program.programId,
  ...
  async derivePredecessorVoterWeightRecordPDA(
    realm: PublicKey,
    mint: PublicKey,
    walletPk: PublicKey,
  ): Promise<
    ...
    | undefined
  > {
    const predecessorProgramId = await this.getPredecessorProgramId(
      realm,
      mint,
    );
   ...
   * @param mint
   */
  async derivePredecessorMaxVoterWeightRecordPDA(
    realm: PublicKey,
    mint: PublicKey,
  ): Promise<
    | {
  ...
  > {
    ...
    if (!predecessorProgramId) return undefined;
    return Client.getMaxVoterWeightRecordPDAForProgram(
      realm,
      mint,
      predecessorProgramId,
    );
