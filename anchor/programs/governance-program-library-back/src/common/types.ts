...
type HasPreviousVotingWeightPlugin<T extends IdlDefinedFieldsNamed[number]> =
  T extends [infer First extends IdlDefinedFieldsNamed[number], ...infer Rest]
    ? First['name'] extends RegistrarAccountTemplate['type']['fields'][0]['name']
      ...
      ? Rest extends HasPreviousVotingWeightPlugin<Rest>
        ? T
        : never
      : never
    : never;
...
type MatchesRegistrarAccountType<T extends IdlDefinedFieldsNamed[number]> =
  T['name'] extends 'registrar'
    ? T extends HasPreviousVotingWeightPlugin<T>
      ? T
      : never
    : never;

// A type function that checks that an IDLAccountDef array has a registrar account
type HasRegistrar<T extends IdlDefinedFieldsNamed[number]> = T extends [
  infer First extends IdlDefinedFieldsNamed[number],
  ...infer Rest,
]
  ? First extends MatchesRegistrarAccountType<First>
    ? T
    : Rest extends IdlDefinedFieldsNamed[number]
    ? Rest extends HasRegistrar<Rest>
      ? T
      : never
    : never
  : never;
...
export type PluginProgramAccounts<T extends Idl> =
  Program<T>['account'] extends {
