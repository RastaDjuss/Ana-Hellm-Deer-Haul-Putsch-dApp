...
if (!voterString || !realmString || !pluginName) {
  console.error(
    'Usage: updateVoterWeightRecord <voter> <realm> <mint> <pluginName> [rpcUrl]',
  );
  process.exit(1);
}
...
(async () => {
  ...
  const { voterWeightPk } = client.getVoterWeightRecordPDA(
    realmPk,
    communityMintPk,
    voterPk,
  );
  ...
  const maxVoterWeight = client.getMaxVoterWeightRecordPDA(
    realmPk,
    communityMintPk,
  );
  if (!maxVoterWeight) {
  ...
  console.log(
    ...
    maxVoterWeight.maxVoterWeightPk.toBase58(),
  );
})().catch((err) => {
  console.error(err);
  process.exit(1);
